...
(function() {
  ...
  function setCurPosition() {
    ...
    function checkMax(v) {
      return v > maxNum ? v - photoItems.length : v;
    }
    toShowItems[0] = curIdx;
    toShowItems[1] = checkMin(curIdx - 1);
    toShowItems[2] = checkMax(curIdx + 1);
  }
  ...
  function showPhoto(dir) {
    ...
    for(var i = 0; i < len; i++) {
      ...
      shownItems.push(num);
      photoItems[num].classList.add(itemClasses[i] + dir);
    }
  }

  function movePhotoPrev(e) {
    var idx;
    if(curIdx === 0) return;
    idx = curIdx - 1;
    updateCurrent(idx);
  ...
  function movePhotoNext(e) {
    var idx;
    if(curIdx === maxNum) return;
    idx = curIdx + 1;
    updateCurrent(idx);
    showPhoto('-for');
  }
  ...
  function init() {
    var startIdx = 0;
    if(location.hash) {
      startIdx = Number(location.hash.replace('#', ''));
    }

    maxNum = photoItems.length - 1;
    if(maxNum > 4) {
      more = true;
    }
